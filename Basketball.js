...
import { Body, Title, List, ListItem, Icon, Item, Button} from 'native-base';
...
class Basketball extends Component {
  ...
  componentDidMount() {
    this.interval = setInterval(this.update.bind(this), 1000 / 60);
     this.refs.toast.show('Lets start the game!!')
  }
    ...
    if (this.state.scored === null) {
      if (this.state.y + radius > NET_Y + NET_HEIGHT / 2 && nextState.y + radius < NET_Y + NET_HEIGHT / 2) {
        if (nextState.x + radius > NET_LEFT_BORDER_X && nextState.x + radius < NET_RIGHT_BORDER_X) {
          ...
          nextState.score = nextState.score;
        }
      }
    }
  }
  ...
  render() {
    return (
      <View style={this.state.score % 5 == 0 ? styles.container : styles.container1}>
        
        <Score y={FLOOR_HEIGHT * 3} score={this.state.score} scored={this.state.scored} 
          style={this.state.score % 5 == 0 ? styles.score : styles.scoreMLT10}/>
        ...
        <Text style={this.state.levelUp === true && this.state.score % 5 == 0 ? styles.levelUpText : styles.levelText }>Level Up!!</Text>
        </View>

        {(this.state.score % 5 == 0 && this.state.levelUp == false && this.state.score > 0 ) | (this.state.score % 5 == 0 && this.state.score > 0 )
          ? <View style={{marginLeft:5,backgroundColor:"#DAA520", width:50, alignItems:"center", padding:5}}>
              <TouchableOpacity onPress={this._toggleModal}>
                <Icon type="FontAwesome" name='heart' style={styles.starActive} />
                <Icon type="FontAwesome" name='heart' style={styles.starActive} />
                <Icon type="FontAwesome" name='heart' style={styles.starActive} />
              </TouchableOpacity>
            </View>
        ...
        <Toast ref="toast"
             position='center'
             style={{backgroundColor:'#FFD700', width: 250, height:50}}
             textStyle={{color:'#663399', textAlign:"center", justifyContent:"center", fontSize:20, fontWeight:'800'}}
